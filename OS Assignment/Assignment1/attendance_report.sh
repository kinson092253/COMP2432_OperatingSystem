...
# after input the data file name, "employee" also MUST input to seperate 
# the emplyee data
# which also count as 1 variable
# the third variable is employee data which allow user to input at least 
# one emplyoee ID or name
if [ "$#" -lt 3 ]; then
    echo "Input Format: $0 <HR_file | Sales_file | hkpolyu*> employee [employee_ids | employee_names...]"
    exit 1
fi
...
# the second checking for user who MUST input employee ID or name after 
# "employee"
# the $# will count the total file name varialbe user input
# the add 1 mean after data file name, add the "employee"
# if the condition is true, that means user they do not input any data 
# after "employee"
...
# use global variable to save the user input after "employee" as the 
# employee data
EMPLOYEE_INPUTS=("${@:${#DATA_FILES[@]}+1}")



# after get all employee inputs, create two array for checking the input 
# is valid or not
EMPLOYEE_DATA=()
INVALID_INPUTS=()
...
# if the ID which is exist in employees.dat then save it in EMPLOYEE_IDS 
# array, if not, save it in INVALID_INPUTS array
for EMPLOYEE_INPUT in "${EMPLOYEE_INPUTS[@]}"; do
	...
	# then seperate it will two different variable EMPLOYEE_ID and 
        # EMPLOYEE_NAME
...
MARKETING_LEAVE=0
	...
	FOUND_ATTENDANCE=0
        ...
        if [[ -f "$DATA_FILE" ]]; then
            ...
            done < "$DATA_FILE"  # which mean the $attendance will load 
				 # the data from $DATA_FILE
